...
define(['q'], function(Q) {
     ...
     */
    function replaceAssetRefsWithUri(html, tags, attribute, zimFile, callback) {
        ...
        assetsCache.assetsCount = 0;
        assetsCache.assetsFulfilled = 0;
        titles.forEach(function(title) {
            getItem(zimFile + '@' + title[1], function(assetContent) {
                assetsCache.assetsCount++;
                if (assetContent || assetContent === '') {
                    assetsCache.assetsFulfilled++;
                    var tag = title[0].match(/^<([^\s]+)/)[1];
                    var mimetype = /type=["']\s*([^"'\s]+)/.exec(title[0]);
                    ...
                    if (tag === 'link') {
                        ...
                        html = html.replace(title[0], 
                            '<style ' + attribute + '="' + title[1] + '" type="' + mimetype + '">' + assetContent + '</style>');
                    } else {
                        mimetype = mimetype && tag === 'script' ? mimetype : 'text/javascript';
                        var assetBlob = new Blob([assetContent], { type: mimetype });
                        var assetUri = URL.createObjectURL(assetBlob);
                        var refAttribute = tag === 'script' ? 'src' : 'href';
                        var newAsset = title[0].replace(attribute, refAttribute);
                        newAsset = newAsset.replace(title[1], assetUri);
                        newAsset = newAsset.replace(/>/, ' data-kiwixsrc="' + title[1] + '">');
                        html = html.replace(title[0], newAsset);
                    }
                }
                if (assetsCache.assetsCount >= titles.length) {
                    assetsCache.assetsResolved = true;
                    callback(html);
                }
            });
        });
