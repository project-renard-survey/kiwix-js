...
define(['jquery', 'zimArchiveLoader', 'util', 'uiUtil', 'cache', 'utf8', 'cookies','abstractFilesystemAccess','q'],
 function($, zimArchiveLoader, util, uiUtil, cache, utf8, cookies, abstractFilesystemAccess, q) {
                ...
                var readFile = function(dirEntry, callback) {
                    if (dirEntry === null) {
                        ...
                        selectedArchive.readBinaryFile(dirEntry, function(fileDirEntry, content) {
                            // We have to parse the content now because buffer will be fully transferred to SW
                            var utf8content = callback && /\.html$|\.css$|\.js$/i.test(title) ? utf8.parse(content) : '';
                            ...
                            if (callback) callback(fileDirEntry, utf8content);
                        });
                    }
                };
                // Cache intercept for files of type .html, .css, .js (DEV: extend to include others if they are to be cached)
                // Sends cached content if it exists, or caches the returned content from readFile() 
                if (/\.html$|\.css$|\.js$/i.test(title)) {
                    cache.getItem(selectedArchive._file._files[0].name + '@' + title, function(response) {
                        if (response) {
                            ...
                            if (/\.css$|\.js$/i.test(title) && !/cacheAssets=false\b/i.test(document.cookie)) $('#cachingCSS').show();
                            selectedArchive.getDirEntryByTitle(title)
                            .then(function(dirEntry) {
                                return readFile(dirEntry, function(fileDirEntry, content) {
                                    $('#cachingCSS').hide();
                                    if (!content) return; // DirEntry was a redirect
    ...
    function displayArticleContentInIframe(dirEntry, htmlArticle) {
        ...
        // Check to see if the cache contains any of the JS or CSS asset titles, and if so, pre-load them
        // NB Because this is async, it must be the last manipulation of htmlArticle before injection
        cache.replaceAssetRefsWithUri(htmlArticle, 'link|script', 'data-kiwixurl', dirEntry._zimfile._files[0].name, 
            function(result) {
                htmlArticle = result;
            }
        );
        ...
        cache.wait('assetsResolved').then(function() {
